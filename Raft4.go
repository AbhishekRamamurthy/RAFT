...
func ProcessMessage(message string) string{
		...
		if strings.Contains(message,"RequestVote 5001") {
			...
			leader="5004"
			message="Ack Leader"
		} else if strings.Contains(message,"HeartBeat") && leader != "5003"{
...
func RequestForVote(conn net.Conn,ports string) {
	...
	if voteCount >= memberCount-1 && voteTo =="5003"{
		mutex.Lock()
	...
	} else if voteCount >= memberCount-1 && leaderDead != "" && voteTo == "5003 "{
