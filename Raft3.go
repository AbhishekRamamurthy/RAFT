...
func ProcessMessage(message string) string{
		...
		if strings.Contains(message,"RequestVote 5000") {
		...
		} else if strings.Contains(message,"RequestVote 5001") {
			...
			leader="5004"
			message="Ack Leader"
		} else if strings.Contains(message,"HeartBeat") && leader != "5002"{
...
func RequestForVote(conn net.Conn,ports string) {
	...
	if voteCount >= memberCount-1 && voteTo == "5002"{
		mutex.Lock()
	...
	} else if voteCount >= memberCount-1 && leaderDead != "" && voteTo == "5002" {
    ...
    for {
        if leaderDead != "" {
            if leaderDead == ports {
                    ...
                    memberCount--
                    mutex.Unlock()
				    fmt.Println("link old leader closed")
                   conn.Close()
                    break
            } else {
                if voteTo == "" || voteTo == "5002"{
